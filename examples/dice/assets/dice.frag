...
void main() {
  ...
  if (gl_FrontFacing) {
    outColor = vec4(fragColor.r*i,fragColor.g*i,fragColor.b*i,2.0f); //por fora, tom entre branco e cinza
    // outColor = fragColor;
  } else {
    outColor = vec4(0.5f, 0.5f, 0.5f, 2.0f); //por dentro, tom de cinza escuro
  }
}